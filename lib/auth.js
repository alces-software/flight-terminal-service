...
function authenticate(socket, authData, callback) {
  jwt.verify(authData.jwt, JSON_WEB_TOKEN_SECRET, (err, ssoUser) => {
    if (err) {
      ...
      fetchTerminalServicesConfig(authData.jwt, authData.siteId)
...
function reconfigure(store) {
  ...
  return fetchTerminalServicesConfig(authData.jwt, authData.siteId)
    .then((terminalServiceConfig) => {
      return configure(
        authData,
        terminalServiceConfig,
...
function configure(authData, terminalServiceConfig, ssoUser, store) {
  if (terminalServiceConfig.flight_directory_config != null) {
    storeAuthData(store, authData, ssoUser);
    configureCmd(store, terminalServiceConfig.flight_directory_config, ssoUser);
    return true;
  } else {
    return false;
  }
...
function configureCmd(store, flightDirectoryConfig, ssoUser) {
  ...
  const [hostname, port=22] = flightDirectoryConfig.hostname.split(':');
